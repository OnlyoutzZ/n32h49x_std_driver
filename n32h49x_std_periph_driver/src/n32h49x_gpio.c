...
void GPIO_DeInitPin(GPIO_Module* GPIOx, uint32_t Pin)
{
    ...
    pExtiCfg = (uint32_t*)(&(AFIO->EXTI_CFG1));
    ...
    while((Pin >> pos) != 0U)
    {
        ...
        if(CurrentPin != 0UL)
        {
            ...
            for(i=0; i<16U; i++)
            {
                tempReg = (pExtiCfg[i >> 2UL] >> ((i & 0x03UL) * 8UL)) & AFIO_EXTI_BIT_MASK;
                if( tempReg == tempAF )
                {
                    pExtiCfg[i >> 2UL] &= ~(AFIO_EXTI_BIT_MASK << ((i & 0x03UL) * 8UL));
                    break;
                }
            }
...
{
    RCC_EnableAPB2PeriphReset(RCC_APB2_PERIPHEN_AFIO);
}
 ...
 *\*\return none
 *\*\note  If PC13,PC14,PC15 used as GPIO or Alternate IO, the clock of GPIOF must be enabled:
 *\*\           Set IOPFEN(RCC_AHB1PCLKEN register bit[5]) bit to '1'. 
 */
...
void AFIO_ConfigPinAnalogSignalChannel(GPIO_Module* GPIOx, uint32_t Pin,FunctionalState cmd)
{
   ...
   pAnaenCfg[RegNum] = tempReg;
}

/**
 *\*\name   SDRAM_EnableClockInversion.
 *\*\fun    Enable or disable the clock inversion of SDRAM.
 *\*\param  cmd :
 *\*\          - DISABLE
 *\*\          - ENABLE
 *\*\return none
...
void SDRAM_EnableClockInversion(FunctionalState cmd)
{
    ...
    if(cmd != DISABLE)
    {
        tempReg &= ~(AFIO_ANAEN_CFG4_CLKINVEN);
    }
    else
    {
        tempReg |= AFIO_ANAEN_CFG4_CLKINVEN;
...
void AFIO_ConfigPinFilter(GPIO_Module* GPIOx, uint32_t Pin,FunctionalState cmd)
{
    ...
    pPinFilter = (uint32_t *)(&(AFIO->FILTER_CFG1));
    GPIO_Index = (uint32_t)GPIOx;
    GPIO_Index = GPIO_GET_INDEX(GPIO_Index);

    RegNum = GPIO_Index / 2UL;
    BitMask = Pin << (16UL * (GPIO_Index & 0x1UL));
    tempReg = pPinFilter[RegNum];
