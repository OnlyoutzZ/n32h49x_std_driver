...
{
    RCC_EnableAPB1PeriphReset(RCC_APB1_PERIPHRST_DAC12);
}
...
void DAC_DmaEnable(DACX DACx, FunctionalState Cmd)
{
    if(IS_DAC1(DACx))
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACx_CTRL_DMA_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACx_CTRL_DMA_EN_MASK);
    ...
    else
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACy_CTRL_DMA_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACy_CTRL_DMA_EN_MASK);
...
void DAC_SoftTrgEnable(DACX DACx, FunctionalState Cmd)
{
    if(IS_DAC1(DACx))
    {
        if (Cmd != DISABLE)
        {
            DAC12->SOTTR |= (DACx_CTRL_TRIGxEN_MASK);
        }
        else
        {
            DAC12->SOTTR &= ~(DACx_CTRL_TRIGxEN_MASK);
    ...
    else
    {
        if (Cmd != DISABLE)
        {
            DAC12->SOTTR |= (DACy_CTRL_TRIGyEN_MASK);
        }
        else
        {
            DAC12->SOTTR &= ~(DACy_CTRL_TRIGyEN_MASK);
...
void DAC_SoftTrgSawStepEnable(DACX DACx, FunctionalState Cmd)
{
    if(IS_DAC1(DACx))
    {
        if (Cmd != DISABLE)
        {
            DAC12->SOTTR |= (DACx_CTRL_TRIGxSTEPEN_MASK);
        }
        else
        {
            DAC12->SOTTR &= ~(DACx_CTRL_TRIGxSTEPEN_MASK);
    ...
    else
    {
        if (Cmd != DISABLE)
        {
            DAC12->SOTTR |= (DACy_CTRL_TRIGySTEPEN_MASK);
        }
        else
        {
            DAC12->SOTTR &= ~(DACy_CTRL_TRIGySTEPEN_MASK);
...
void DAC_DualSoftwareTrgEnable(DAC_Module *Dual_DACx, FunctionalState Cmd)
{
    if (Cmd != DISABLE)
    {
        ...
        DAC12->SOTTR |= DACx_CTRL_TRIGALLEN_MASK;
    }
    else
    {
...
void DAC_DualSoftwareTrgSawStepEnable(DAC_Module *Dual_DACx, FunctionalState Cmd)
{
    if (Cmd != DISABLE)
    {
        ...
        DAC12->SOTTR |= DACy_CTRL_TRIGALLSTEPEN_MASK;
    }
    else
    {
...
void DAC_CaliEnable(DACX DACx, FunctionalState Cmd)
{
    if(IS_DAC1(DACx))
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACx_CTRL_CALIBATE_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACx_CTRL_CALIBATE_EN_MASK);
    ...
    else
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACy_CTRL_CALIBATE_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACy_CTRL_CALIBATE_EN_MASK);
...
void DAC_ConnetToOnChipEnable(DACX DACx, FunctionalState Cmd)
{
    if(IS_DAC1(DACx))
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACx_CTRL_OUTPUT_TO_ONCHAIP_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACx_CTRL_OUTPUT_TO_ONCHAIP_EN_MASK);
    ...
    else
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACy_CTRL_OUTPUT_TO_ONCHAIP_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACy_CTRL_OUTPUT_TO_ONCHAIP_EN_MASK);
...
void DAC_ConnetToExternalPinEnable(DACX DACx, FunctionalState Cmd)
{
    if(IS_DAC1(DACx))
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACx_CTRL_OUTPUT_TO_EXTPIN_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACx_CTRL_OUTPUT_TO_EXTPIN_EN_MASK);
    ...
    else
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACy_CTRL_OUTPUT_TO_EXTPIN_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACy_CTRL_OUTPUT_TO_EXTPIN_EN_MASK);
...
void DAC_DMADoubleDataModeEnable(DACX DACx, FunctionalState Cmd)
{
    if(IS_DAC1(DACx))
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACx_CTRL_DMADOUBLEMODE_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACx_CTRL_DMADOUBLEMODE_EN_MASK);
    ...
    else
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACy_CTRL_DMADOUBLEMODE_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACy_CTRL_DMADOUBLEMODE_EN_MASK);
...
void DAC_SignFormatModeEnable(DACX DACx, FunctionalState Cmd)
{
    if(IS_DAC1(DACx))
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACx_CTRL_SINGFORMAT_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACx_CTRL_SINGFORMAT_EN_MASK);
    ...
    else
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACy_CTRL_SINGFORMAT_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACy_CTRL_SINGFORMAT_EN_MASK);
...
void DAC_HighDriveAbilityEnable(DACX DACx, FunctionalState Cmd)
{
    if(IS_DAC1(DACx))
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACx_CTRL_HIGH_DRIVER_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACx_CTRL_HIGH_DRIVER_EN_MASK);
    ...
    else
    {
        if(Cmd != DISABLE)
        {
            DAC12->CTRL |= DACy_CTRL_HIGH_DRIVER_EN_MASK;
        }
        else
        {
            DAC12->CTRL &= ~(DACy_CTRL_HIGH_DRIVER_EN_MASK);
