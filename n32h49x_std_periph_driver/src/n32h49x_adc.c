...
#define ADC_CTRL1_CLR_MASK          ((uint32_t)0x00000F81U)
#define ADC_CTRL2_CLR_MASK          ((uint32_t)0x013F0032U)
#define ADC_CTRL3_CLR_MASK          ((uint32_t)0x00000003U)
...
void ADC_EnableMutiAdcDSMU(ADC_Module *ADCx, FunctionalState Cmd)
{
    if (Cmd != DISABLE)
    {
        if((ADCx->CTRL3&ADC_DSMU_EN_MASK) != ADC_DSMU_EN_MASK )
        {
           /* Enable ADC DSMU mode  firstly*/
            ADCx->CTRL3 |= ADC_DSMU_EN_MASK;
        }
        else
        {
            /* no process */
        ...
        ADCx->CTRL3 |= ADC_MUTI_ADC_DSMU_MASK;
    }
    else
    {
...
/**
*\*\name    ADC_EnableAdcDSMU.
*\*\fun     Configures ADC DSMU mode for the specified ADC enable or disable.
*\*\param   ADCx :
*\*\          - ADC1
*\*\          - ADC2
*\*\          - ADC3
...
*\*\return  none
**/
void ADC_EnableAdcDSMU(ADC_Module *ADCx, FunctionalState Cmd)
{
    if (Cmd != DISABLE)
    {
        /* Enable ADC DSMU mode */
        ADCx->CTRL3 |= ADC_DSMU_EN_MASK;
    }
    else
    {
        /* Disable ADC DSMU mode */
        ADCx->CTRL3 &= (~ADC_DSMU_EN_MASK);
    }
}
...
*\*\          - DISABLE
*\*\return  none
*\*\note The DSMU filtering cannot distinguish the difference among ADC channels.
**/
void ADC_SetAdcDSMUChannel(ADC_Module *ADCx, uint8_t ADC_Channel, FunctionalState Cmd)
{
    if (Cmd != DISABLE)
    {
        /* Enable ADC DSMU mode */
        ADCx->DSMUCHMASK &= ~((uint32_t)1 << ADC_Channel);
    }
    else
    {
        /* Disable ADC DSMU mode */
...
void ADC_ConfigAnalogWatchdogThresholds(ADC_Module* ADCx, ADC_AWDG Awdg, uint16_t HighThreshold, uint16_t LowThreshold)
{
    ...
    if(Awdg == ADC_AWDG1)
    {
        ...
        tempreg &= (~ADC_AWD1LOW_LTH);
        tempreg |= (LowThreshold&ADC_AWD1LOW_LTH);
...
INTStatus ADC_GetIntStatus(ADC_Module* ADCx, uint16_t ADC_IT)
{
    ...
    if(((ADCx->STS & ((uint32_t)ADC_IT) )!= 0U) && (enablestatus != 0U))
    {
        /* ADC_IT is set */
        bitstatus = SET;
    }
...
*\*\         - ADC_CTRL3_CKMOD_AHB
*\*\         - ADC_CTRL3_CKMOD_PLL
*\*\         - ADC_CTRL3_CKMOD_USBHSPLL
*\*\param   RCC_ADCPrescaler : specifies the ADC prescaler.
*\*\        When ADC_ClkMode = ADC_CTRL3_CKMOD_AHB: RCC_ADCPrescaler can be on of the following values:
*\*\         - RCC_ADCHCLK_DIV1        
*\*\         - RCC_ADCHCLK_DIV2        
*\*\         - RCC_ADCHCLK_DIV3        
*\*\         - RCC_ADCHCLK_DIV4        
...
*\*\         - RCC_ADCHCLK_DIV16       
*\*\         - RCC_ADCHCLK_DIV32  
*\*\        When ADC_ClkMode = ADC_CTRL3_CKMOD_PLL or ADC_CTRL3_CKMOD_USBHSPLL: RCC_ADCPrescaler can be on of the following values:
*\*\         - RCC_ADCPLLCLK_DIV1 
*\*\         - RCC_ADCPLLCLK_DIV2 
*\*\         - RCC_ADCPLLCLK_DIV3 
*\*\         - RCC_ADCPLLCLK_DIV4 
...
void ADC_ConfigClk(ADC_CTRL3_CKMOD ADC_ClkMode, uint32_t RCC_ADCPrescaler)
{
    ...
    {
        if(ADC_ClkMode == ADC_CTRL3_CKMOD_PLL)
        {
            RCC_ConfigAdcPllClk(RCC_ADCPLLCLK_SRC_PLL,RCC_ADCPrescaler, RCC_ADCPLLCLK_ENABLE);
        }
        else
        {
            RCC_ConfigAdcPllClk(RCC_ADCPLLCLK_SRC_USBHS240M,RCC_ADCPrescaler, RCC_ADCPLLCLK_ENABLE);
        }
