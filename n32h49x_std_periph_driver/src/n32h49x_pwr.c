...
{
    RCC_EnableAPB1PeriphReset(RCC_APB1_PERIPHRST_PWR);
}
...
void PWR_PVDINPinSelect(PVD_IN_SEL PVD_pin)
{
   ...
   {
       /* Set PVD_IN bit */
       *(uint32_t *)0x40010000 |= 0x00000002U;
   }
   else
   {
       /* Reset PVD_IN bit */
       *(uint32_t *)0x40010000 &= 0xFFFFFFFDU;
   }  
}
...
*\*\          - WAKEUP_PIN5EN   WKUP5(PD2)
*\*\          - WAKEUP_PIN6EN   WKUP6(PD3)
*\*\param   Cmd: 
*\*\          - ENABLE  
*\*\          - DISABLE 
*\*\return  none
...
*\*\          - WAKEUP_PIN5POL   WKUP5(PD2)
*\*\          - WAKEUP_PIN6POL   WKUP6(PD3)
...
*\*\       - PWR_CLR_WKUP5     PD2 Wake Up flag
*\*\       - PWR_CLR_WKUP6     PD3 Wake Up flag
...
/**
*\*\name   PWR_EnableNRSTOut.
*\*\fun    Enable or disable the output function of NRST Pin.
*\*\param   Cmd: 
*\*\          - ENABLE  
*\*\          - DISABLE   
*\*\return  none. 
**/
void PWR_EnableNRSTOut(FunctionalState Cmd)
{
    if(Cmd != DISABLE)
    {
        /* Set NRSTOUTBP bit */
        PWR->CTRL6 |= PWR_CTRL6_NRSTOUTBP;
    }
    else
    {
        /* Reset NRSTOUTBP bit */
...
/**
*\*\name   PWR_ConfigNRSTDigitalFilterWidth.
*\*\fun    Configuring the NRST digital filter width.
*\*\param  NRST_dfw: 
*\*\        - 0x0 ~ 0xFF  
*\*\return  none. 
*\*\note  digital filter width =  sysclk * NRST_dfw. 
**/
void PWR_ConfigNRSTDigitalFilterWidth(uint32_t NRST_dfw)
{
    uint32_t temp_value;

    temp_value = PWR->NRSTCTRL;
    /* Clear NRSTDFW[7:0] bits */
    temp_value &= PWR_NRST_DFW_MASK;
    /* Set NRSTDFW[7:0] bits according to NRST_dfw value */
    temp_value |= ((NRST_dfw & 0xFF) << 4);

    /* Store the new value */
    PWR->NRSTCTRL = temp_value;
}


/**
*\*\name    PWR_EnableNRSTDigitalFilter.
*\*\fun     Enable or disable NRST digital filter 
*\*\param   Cmd: 
*\*\          - ENABLE  
*\*\          - DISABLE   
*\*\return  none
**/
void PWR_EnableNRSTDigitalFilter(FunctionalState Cmd)
{ 
   if(Cmd != DISABLE)
   {
       /* Reset NRSTDFEN bit */
       PWR->NRSTCTRL &= (~PWR_DF_ENABLE);
   }
   else
   {
       /* Set NRSTDFEN bit */
...
/**
*\*\name    PWR_EnableNRSTAnalogFilter.
*\*\fun     Enable or disable NRST Analog filter 
*\*\param   Cmd: 
*\*\          - ENABLE  
*\*\          - DISABLE   
*\*\return  none
**/
void PWR_EnableNRSTAnalogFilter(FunctionalState Cmd)
{ 
   if(Cmd != DISABLE)
   {
       /* Reset NRSTAFEN bit */
       PWR->NRSTCTRL &= (~PWR_AF_ENABLE);
   }
   else
   {
       /* Set NRSTAFEN bit */
