...
#define ETH_SWRESET_TIMEOUT                     ((uint32_t)0x0000F000U)
...
#define ETH_WRITE_REG_DELAY                     ((uint32_t)500U)

/* ETH External PHY Configuration Delay */
#define ETH_PHY_CONFIG_DELAY                    ((uint32_t)500U)
...
EthFuncStatusType ETH_Init(ETH_Module* ETHx, ETH_InfoType* pInfo, ETH_InitType* ETH_InitParam)
{
    ...
    TempValue = ((uint32_t)(ETH_InitParam->Promiscuous)        | (uint32_t)(ETH_InitParam->HashUnicast)
               ...
               | (uint32_t)(ETH_InitParam->VLANTagFilter)      | (uint32_t)(ETH_InitParam->ReceiveAll));
    ...
    TempValue = (uint32_t)(ETH_InitParam->TxRxArbitration);
    ...
    TempValue = ((uint32_t)(ETH_InitParam->PBLx8mode) | (uint32_t)(ETH_InitParam->DescriptorSkipLen));
    ...
    TempValue = ((uint32_t)(ETH_InitParam->OperateSecondPacket) | (uint32_t)(ETH_InitParam->TxBurstLength)
               | (uint32_t)(ETH_InitParam->EarlyTxInterruptCtrl));
...
EthFuncStatusType ETH_BuildRxDescriptors(ETH_Module* ETHx, ETH_InfoType* pInfo)
{
    ...
    if ((ETH->DMACH0STS & ETH_DMACH0STS_RBU) != (uint32_t)RESET)
    {
        /* Clear RBUS ETHERNET DMA flag */
        ETH->DMACH0STS = ETH_DMACH0STS_RBU;
...
EthFuncStatusType ETH_PrepareTxDescriptors(ETH_Module* ETHx, ETH_InfoType* pInfo, ETH_TxPacketType* pTxPacket, uint32_t ItMode)
{
    ...
    {
        // N32H49x does not support VLAN insertion or replacemen
        ...
        {
            // N32H49x does not support double VLAN processing
        }
    }
    ...
    {
        // N32H49x does not support TCP segementation
    }
    ...
    if ((pTxPacket->Attributes & (uint32_t)ETH_TX_PACKETS_FEATURES_VLANTAG) != (uint32_t)RESET)
    {
        // N32H49x does not support SA and VLAN Insertion on Tx option
    }
...
EthFuncStatusType ETH_GetRxDataBuffer(ETH_InfoType* pInfo, ETH_BufferType* pRxBuffer)
{
    ...
    /* Get Address of the last buffer address */
    pRxBuf->pBuf = (uint8_t*)pRxDescriptor->Buf1Addr;
...
EthFuncStatusType ETH_GetRxPacketInfo(ETH_InfoType* pInfo, ETH_RxPacketType* pRxPacket)
{
    ...
    else
    {
        if ((pRxDescriptor->DESC3 & ETH_DMARXND3WBF_RS0V) != (uint32_t)RESET)
        {
            ...
            {
                // N32H49x does not support Double VLAN Tag
            }
            else
            {
                /* Get Outer VLAN Tag */
