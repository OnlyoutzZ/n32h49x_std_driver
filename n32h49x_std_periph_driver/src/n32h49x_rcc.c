...
void RCC_EnablePll(FunctionalState Cmd)
{
   ...
   {
       /* Set PLLEN bit */
       RCC->PLLCTRL |= RCC_PLLCTRL_PLLRSTEN;
...
/**
*\*\name    RCC_ConfigEXTPll.
*\*\fun     Configures the EXTPLL clock source and multiplication factor.
*\*\param   EXTPLL_source(EXTPLL entry clock source):
*\*\   		  - RCC_EXTPLL_SRC_HSI         HSI oscillator clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_HSI_DIV2    HSI oscillator divider 2 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_HSE         HSE oscillator clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_HSE_DIV2    HSE oscillator divider 2 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV16   PLL divider 16 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV17   PLL divider 17 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV18   PLL divider 18 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV19   PLL divider 19 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV20   PLL divider 20 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV21   PLL divider 21 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV22   PLL divider 22 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV23   PLL divider 23 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV24   PLL divider 24 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV25   PLL divider 25 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV26   PLL divider 26 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV27   PLL divider 27 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV28   PLL divider 28 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV29   PLL divider 29 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV30   PLL divider 30 clock selected as EXTPLL clock entry
*\*\   		  - RCC_EXTPLL_SRC_PLL_DIV31   PLL divider 31 clock selected as EXTPLL clock entry
*\*\param   EXTPLL_prescaler(EXTPLL input divider):
*\*\	      - RCC_EXTPLL_PRE_1 
*\*\	      - RCC_EXTPLL_PRE_2
*\*\	      - RCC_EXTPLL_PRE_3 
*\*\	      - RCC_EXTPLL_PRE_4 
*\*\param   EXTPLL_mul(EXTPLL multiplication factor):
*\*\	      - RCC_EXTPLL_MUL_8  
*\*\	      - RCC_EXTPLL_MUL_10 
*\*\	      - RCC_EXTPLL_MUL_12 
*\*\	      - RCC_EXTPLL_MUL_14 
...
*\*\	      - RCC_EXTPLL_MUL_74 
*\*\	      - RCC_EXTPLL_MUL_76 
*\*\param   EXTPLL_outdivider(EXTPLL out divider):
*\*\	      - RCC_EXTPLLOUT_DIV_1 
*\*\	      - RCC_EXTPLLOUT_DIV_2
*\*\	      - RCC_EXTPLLOUT_DIV_4 
*\*\	      - RCC_EXTPLLOUT_DIV_8 
*\*\return  none
*\*\note    This function must be used only when the EXTPLL is disabled. 
*\*\note    Fin frequency requirement is in the range of 4MHz ~ 50MHz,
*\*\	    Fref frequency requirement is in the range of 4MHz ~ 25MHz,
*\*\	    Fvco frequency requirement is in the range of 64MHz ~ 500MHz. 
**/
void RCC_ConfigEXTPll(uint32_t EXTPLL_source, uint32_t  EXTPLL_prescaler, uint32_t EXTPLL_mul, uint32_t EXTPLL_outdivider)
{
    uint32_t temp_value1,temp_value2;  
    
    /* get the EXTpll value */
    temp_value1 = RCC->PLLCTRL;
    temp_value2 = RCC->EXTCTRL;

    /* Clear EXTPLLPRES bits */
    temp_value1 &= RCC_PLLCTRL_EXTPLL_MASK;
    /* Clear EXTPLLSRC, EXTHSEPRES, EXTHSIPRES and EXTPLL[5:0] */
    /* Clear EXTINPRES[1:0], EXTOUTPRES[1:0],LPFC2[2:0],LPFR[3:0],LPFC1[3:0] bits */
    temp_value2 &= RCC_EXTCTRL_EXTPLL_MASK;

    if(EXTPLL_source & 0x1) 
    {
        temp_value2 |= (EXTPLL_source & 0xFFFFFFFE);/* select HSI or HSE */
    }
    else
    {
        temp_value1 |= EXTPLL_source;
        temp_value2 |= RCC_EXTCTRL_EXTPLLSRC_1;/* select PLL */
    }

    if(EXTPLL_mul <= RCC_EXTPLL_MUL_24)
    {
        temp_value2 |= (EXTPLL_prescaler | EXTPLL_mul | EXTPLL_outdivider | ((uint32_t)0x00006340));
    }
    else if(EXTPLL_mul <= RCC_EXTPLL_MUL_40)
    {
        temp_value2 |= (EXTPLL_prescaler | EXTPLL_mul | EXTPLL_outdivider | ((uint32_t)0x00006740));
    }
    else
    {
        temp_value2 |= (EXTPLL_prescaler | EXTPLL_mul | EXTPLL_outdivider | ((uint32_t)0x00006C40));
    }

    /* Store the new value */
    RCC->PLLCTRL  = temp_value1;
    RCC->EXTCTRL  = temp_value2;
}

/**
*\*\name    RCC_EnableEXTPll.
*\*\fun     Enables the EXTPLL.
*\*\param   Cmd: 
*\*\          - ENABLE  
*\*\          - DISABLE 
*\*\return  none
*\*\note  The RCC_CTRL.PLLEN bit must be enabled when using EXTPLL.
**/ 
void RCC_EnableEXTPll(FunctionalState Cmd)
{
    if(Cmd == ENABLE)
   {
       /* Set EXTPLLEN bit */
       RCC->PLLCTRL |= RCC_PLLCTRL_EXTRSTEN;
       RCC->EXTCTRL |= RCC_EXTCTRL_EXTPLLEN;
   }
   else
   {
       /* Reset EXTPLLEN bit */
...
*\*\	     - RCC_SYSCLK_SRC_EXTPLL        SHRPLL selected as system clock
...
void RCC_ConfigSysclk(uint32_t sysclk_source)
{
    ...
    /* Set SCLKSW[2:0] bits in CFG register */
...
*\*\	     - RCC_CFG_SRC_EXTPLL     SHRPLL selected as system clock
*\*\	     - RCC_CFG_SRC_USBHS240M  USBHS240M selected as system clock
**/
...
*\*\return  none
*\*\note    The RNGCCLK must not exceed interface clock(config by RCC_ConfigPeriphClk()). 
**/
...
*\*\         - RCC_FDCAN_CLKSRC_EXTPLL
*\*\         - RCC_FDCAN_CLKSRC_HSE    
*\*\return  none
*\*\note  The divider needs to be configured first when selecting the PLL or EXTPLL
**/
...
/**
*\*\name    RCC_ConfigSDRAMClk.
*\*\fun     Configures the SDRAM HCLK prescaler.
*\*\param   SDRAM_prescaler(SDRAM HCLK prescaler):
*\*\         - RCC_SDRAM_HCLK_DIV1
*\*\         - RCC_SDRAM_HCLK_DIV2
*\*\         - RCC_SDRAM_HCLK_DIV3
*\*\         - RCC_SDRAM_HCLK_DIV4
...
*\*\return  none
**/
void RCC_ConfigSDRAMClk(uint32_t SDRAM_prescaler)
{
    uint32_t temp_value;

    temp_value = RCC->CFG2;
    /* Clear SDRAMPRES[2:0] bits */
    temp_value &= RCC_SDRAM_HCLK_DIV_MASK;
    /* Set SDRAMPRES[2:0] bits according to SDRAM_prescaler value */
    temp_value |= SDRAM_prescaler;
...
/**
*\*\name    RCC_ConfigDSMUFilterClk.
*\*\fun     Configures the DSMU filter prescaler.
*\*\param   DSMU_prescaler(DSMU filter prescaler):
*\*\         - RCC_DSMU_FLT_DIV1
*\*\         - RCC_DSMU_FLT_DIV2
*\*\         - RCC_DSMU_FLT_DIV4
*\*\         - RCC_DSMU_FLT_DIV6
*\*\return  none
**/
void RCC_ConfigDSMUFilterClk(uint32_t DSMU_prescaler)
{
    uint32_t temp_value;

    temp_value = RCC->CFG2;
    /* Clear DSMUFLTPRES[1:0] bits */
    temp_value &= RCC_DSMU_FLT_DIV_MASK;
    /* Set DSMUFLTPRES[1:0] bits according to DSMU_prescaler value */
    temp_value |= DSMU_prescaler;
...
*\*\fun     Configures the SAC/SDIO/USBHS/ETH/FEMC/XSPI/CORDIC Peripheral HCLK prescaler.
*\*\param   Periph_prescaler(Periph HCLK prescaler):
*\*\         - RCC_PERIPH_HCLK_DIV1
*\*\         - RCC_PERIPH_HCLK_DIV2
*\*\return  none
*\*\note    The interface clock must not exceed 120 MHz. 
**/
void RCC_ConfigPeriphClk(uint32_t Periph_prescaler)
{
    uint32_t temp_value;

    temp_value = RCC->CFG2;
    /* Clear HCLKPRES bits */
    temp_value &= RCC_PERIPH_HCLK_DIV_MASK;
    /* Set HCLKPRES bits according to Periph_prescaler value */
    temp_value |= Periph_prescaler;
...
{
    uint32_t temp, pllclk, extpllclk, pllmull, pllpre, plloutdiv, pllsource, presc;
    ...
    /* Get PLL clock source, Pre-Divider, out div and multiplication factor ----------------------*/
    ...
    /* Get EXTPLL clock source, Pre-Divider, out div and multiplication factor ----------------------*/
    pllmull   = (((RCC->EXTCTRL & RCC_EXTCTRL_EXTPLLMUL) >> 24) * 2) + 4;
    pllpre    = ((RCC->EXTCTRL & RCC_EXTCTRL_EXTINPRES) >> 16) + 1;
    plloutdiv = ((RCC->EXTCTRL & RCC_EXTCTRL_EXTOUTPRES) >> 18);
    pllsource = ((RCC->EXTCTRL & RCC_EXTCTRL_EXTPLLSRC) >> 20);

    if (pllsource == 0x00)
    { 
        /* HSI selected as PLL clock entry */
        extpllclk = HSI_VALUE >> ((RCC->EXTCTRL & RCC_EXTCTRL_EXTHSIPRES) >> 23);
    }
    else if (pllsource == 0x01)
    {
        /* HSE selected as PLL clock entry */
        extpllclk = HSE_VALUE >> ((RCC->EXTCTRL & RCC_EXTCTRL_EXTHSEPRES) >> 22);
    }
    else 
    {
         /* HSE selected as PLL clock entry */
        extpllclk = pllclk / (((RCC->PLLCTRL & RCC_PLLCTRL_EXTPLLPRES) >> 28) + 16);
    }
    extpllclk = (extpllclk / pllpre * pllmull) >> plloutdiv;
    RCC_clocks->ExtpllFreq = extpllclk;
    ...
    else if(temp == RCC_CFG_SCLKSTS_USBHS240M)
    {
        /* HSI used as system clock */
        RCC_clocks->SysclkFreq = 24000000U;
    }
    ...
    else if(temp == RCC_CFG_SCLKSTS_EXTPLL)
    {
        /* PLL used as system clock */
        RCC_clocks->SysclkFreq = extpllclk;
    }
    else
    {
        /* HSI used as system clock */
    ...
    if((RCC->CFG2 & RCC_CFG2_ADCOTGPLLSEL) == RCC_CFG2_ADCOTGPLLSEL)
    {
        ...
        RCC_clocks->AdcPllClkFreq = 240000000U / presc;
    }
    else
    {
...
*\*\        - RCC_APB2_PERIPHEN_USART4
*\*\        - RCC_APB2_PERIPHEN_UART10
*\*\        - RCC_APB2_PERIPHEN_USART9
*\*\        - RCC_APB2_PERIPHEN_GTIM10
*\*\        - RCC_APB2_PERIPHEN_GTIM9 
*\*\        - RCC_APB2_PERIPHEN_GTIM8 
*\*\        - RCC_APB2_PERIPHEN_UART7 
*\*\        - RCC_APB2_PERIPHEN_UART6 
*\*\        - RCC_APB2_PERIPHEN_USART1
*\*\        - RCC_APB2_PERIPHEN_SPI1  
*\*\        - RCC_APB2_PERIPHEN_SPI4  
*\*\        - RCC_APB2_PERIPHEN_SPI5  
*\*\        - RCC_APB2_PERIPHEN_SPI6  
*\*\        - RCC_APB2_PERIPHEN_AFIO  
*\*\param   Cmd: 
*\*\          - ENABLE  
*\*\          - DISABLE 
*\*\return none. 
...
*\*\        - RCC_APB1_PERIPHEN_DSMU  
*\*\        - RCC_APB1_PERIPHEN_DAC12 
*\*\        - RCC_APB1_PERIPHEN_WWDG  
*\*\        - RCC_APB1_PERIPHEN_I2C4  
*\*\        - RCC_APB1_PERIPHEN_I2C3  
*\*\        - RCC_APB1_PERIPHEN_BTIM2 
*\*\        - RCC_APB1_PERIPHEN_BTIM1 
*\*\        - RCC_APB1_PERIPHEN_FDCAN3
*\*\        - RCC_APB1_PERIPHEN_FDCAN2
*\*\        - RCC_APB1_PERIPHEN_FDCAN1
*\*\        - RCC_APB1_PERIPHEN_USBFS 
*\*\        - RCC_APB1_PERIPHEN_UCDR  
*\*\        - RCC_APB1_PERIPHEN_GTIM7 
*\*\        - RCC_APB1_PERIPHEN_GTIM6 
*\*\        - RCC_APB1_PERIPHEN_GTIM5 
*\*\        - RCC_APB1_PERIPHEN_GTIM4 
*\*\        - RCC_APB1_PERIPHEN_GTIM3 
*\*\        - RCC_APB1_PERIPHEN_GTIM2 
*\*\        - RCC_APB1_PERIPHEN_GTIM1 
*\*\        - RCC_APB1_PERIPHEN_SPI3  
*\*\        - RCC_APB1_PERIPHEN_SPI2  
*\*\        - RCC_APB1_PERIPHEN_I2C2  
*\*\        - RCC_APB1_PERIPHEN_I2C1  
*\*\        - RCC_APB1_PERIPHEN_UART5 
*\*\        - RCC_APB1_PERIPHEN_UART8 
*\*\        - RCC_APB1_PERIPHEN_USART3
*\*\        - RCC_APB1_PERIPHEN_USART2
*\*\        - RCC_APB1_PERIPHEN_BKP   
*\*\        - RCC_APB1_PERIPHEN_PWR   
*\*\param   Cmd: 
*\*\          - ENABLE  
*\*\          - DISABLE       
*\*\return none. 
...
*\*\        - RCC_APB2_PERIPHRST_USART4
*\*\        - RCC_APB2_PERIPHRST_GTIM10
*\*\        - RCC_APB2_PERIPHRST_GTIM9 
*\*\        - RCC_APB2_PERIPHRST_GTIM8 
*\*\        - RCC_APB2_PERIPHRST_UART7 
*\*\        - RCC_APB2_PERIPHRST_UART6 
*\*\        - RCC_APB2_PERIPHRST_USART1
*\*\        - RCC_APB2_PERIPHRST_SPI1  
*\*\        - RCC_APB2_PERIPHRST_SPI4  
*\*\        - RCC_APB2_PERIPHRST_SPI5  
*\*\        - RCC_APB2_PERIPHRST_SPI6  
*\*\        - RCC_APB2_PERIPHRST_AFIO  
*\*\return none.
**/
...
*\*\        - RCC_APB1_PERIPHRST_WWDG  
*\*\        - RCC_APB1_PERIPHRST_I2C4  
*\*\        - RCC_APB1_PERIPHRST_I2C3  
*\*\        - RCC_APB1_PERIPHRST_DSMU  
*\*\        - RCC_APB1_PERIPHRST_UART10
*\*\        - RCC_APB1_PERIPHRST_USART9
*\*\        - RCC_APB1_PERIPHRST_BTIM2 
*\*\        - RCC_APB1_PERIPHRST_BTIM1 
*\*\        - RCC_APB1_PERIPHRST_FDCAN3
*\*\        - RCC_APB1_PERIPHRST_FDCAN2
*\*\        - RCC_APB1_PERIPHRST_FDCAN1
*\*\        - RCC_APB1_PERIPHRST_USBFS 
*\*\        - RCC_APB1_PERIPHRST_UCDR  
*\*\        - RCC_APB1_PERIPHRST_GTIM7 
*\*\        - RCC_APB1_PERIPHRST_GTIM6 
*\*\        - RCC_APB1_PERIPHRST_GTIM5 
*\*\        - RCC_APB1_PERIPHRST_GTIM4 
*\*\        - RCC_APB1_PERIPHRST_GTIM3 
*\*\        - RCC_APB1_PERIPHRST_GTIM2 
*\*\        - RCC_APB1_PERIPHRST_GTIM1 
*\*\        - RCC_APB1_PERIPHRST_SPI3  
*\*\        - RCC_APB1_PERIPHRST_SPI2  
*\*\        - RCC_APB1_PERIPHRST_I2C2  
*\*\        - RCC_APB1_PERIPHRST_I2C1  
*\*\        - RCC_APB1_PERIPHRST_UART5 
*\*\        - RCC_APB1_PERIPHRST_UART8 
*\*\        - RCC_APB1_PERIPHRST_USART3
*\*\        - RCC_APB1_PERIPHRST_USART2
*\*\        - RCC_APB1_PERIPHRST_BKP   
*\*\        - RCC_APB1_PERIPHRST_PWR     
*\*\return none. 
**/
...
/**
*\*\name   RCC_ConfigMco1.
*\*\fun    Selects the clock source to output on MCO1 pin.
...
*\*\return  none. 
**/
void RCC_ConfigMco1(uint32_t MCO_source, uint32_t MCO_prescaler)
{
   uint32_t temp_value;

   temp_value = RCC->MCOCFG;
   /* Clear MCO1[3:0] and MCO1PRES[3:0] bits */
   temp_value &= RCC_MCO1_MCOCFG_MASK;
    
   temp_value |= (MCO_source|MCO_prescaler);
...
/**
*\*\name    RCC_EnableMco1.
*\*\fun     Enables the MCO1.
*\*\param   Cmd: 
*\*\          - ENABLE  
*\*\          - DISABLE  
*\*\return  none
**/ 
void RCC_EnableMco1(FunctionalState Cmd)
{
    if(Cmd == ENABLE)
   {
       /* Set MCO1EN bit */
       RCC->MCOCFG |= RCC_MCO1_ENABLE;
   }
   else
   {
       /* Reset MCO1EN bit */
...
/**
*\*\name   RCC_ConfigMco2.
*\*\fun    Selects the clock source to output on MCO2 pin.
*\*\param  MCO_source(clock source to output): 
*\*\     - RCC_MCO_SYSCLK   
*\*\     - RCC_MCO_HSI      
*\*\     - RCC_MCO_HSE      
...
*\*\return  none. 
**/
void RCC_ConfigMco2(uint32_t MCO_source, uint32_t MCO_prescaler)
{
   uint32_t temp_value;

   temp_value = RCC->MCOCFG;
   /* Clear MCO2[3:0] and MCO2PRES[3:0] bits */
   temp_value &= RCC_MCO2_MCOCFG_MASK;
    
   temp_value |= ((MCO_source|MCO_prescaler)<<4);
...
/**
*\*\name    RCC_EnableMco2.
*\*\fun     Enables the MCO2.
*\*\param   Cmd: 
*\*\          - ENABLE  
*\*\          - DISABLE  
*\*\return  none
**/ 
void RCC_EnableMco2(FunctionalState Cmd)
{
    if(Cmd == ENABLE)
   {
       /* Set MCO2EN bit */
       RCC->MCOCFG |= RCC_MCO2_ENABLE;
   }
   else
   {
       /* Reset MCO2EN bit */
...
*\*\        - RCC_REMAP_SDRAM     boot from SDRAM
*\*\return  none. 
**/
...
*\*\	      - RCC_FLAG_RAMERSTF      RAM or FLASH ECC error reset flag
...
*\*\	      - RCC_FLAG_APBALIGNF     APB1 and APB2 Subdivided Clock Rising Edge Alignment flag
*\*\	      - RCC_FLAG_VIOPORRSTF    VDDIO POR Reset flag 
*\*\	      - RCC_FLAG_EXTPLLRDF     EXTPLL clock ready
*\*\return  FlagStatus:    
*\*\      	  - SET 
*\*\  	      - RESET
**/
FlagStatus RCC_GetFlagStatus(uint8_t RCC_flag)
{
    ...
    switch(temp_value)
    {
        ...
        case 6:/* The flag to check is in CSSCTRL register */
            ...
            break;
        case 7:/* The flag to check is in CFG register */
            reg_value = RCC->CFG;
            break;
        default:/* The flag to check is in EXTCTRL register */
            reg_value = RCC->EXTCTRL;
            break;
    }
...
/**
*\*\name    RCC_EnableVDDIOPORReset.
*\*\fun     Enables the VDDIO POR Reset request.
*\*\param   Cmd: 
*\*\          - ENABLE  
*\*\          - DISABLE  
*\*\return  none
**/ 
void RCC_EnableVDDIOPORReset(FunctionalState Cmd)
{
    if(Cmd == ENABLE)
   {
       /* Set VIOPOREN bit */
       RCC->CFG |= RCC_CFG_VIOPOREN;
   }
   else
   {
       /* Reset VIOPOREN bit */
